...
class EMB_PT_gen_panel(Panel):
    ...
    def draw(self, context):
        ...
        for n, box in enumerate(props.text_boxes):
            if not box.is_url:
                c1.prop_search(box, 'icon', icons, 'enum_items', text='', icon='NONE' if not box.icon else box.icon)
        ...
        for box in props.text_boxes:
            if not box.is_url:
                ...
                textBox(layout, f'LINK:{box.link}|NAME:{box.name}')
        layout.label(text='—')
...
class EMB_OT_boxes_clipboard(Operator):
    ...
    def execute(self, context):
        ...
        if props.message_type == 'MESSAGE':
            ...
            texts = '\\n'.join([box.text if not box.is_url else f'LINK:{box.link}|NAME:{box.name}' for box in props.text_boxes])
            icons = ','.join([box.icon for box in props.text_boxes])
            sizes = ','.join([str(box.size) for box in props.text_boxes])
            final = separate_chr.join([str(int(time())), props.title, texts, icons, sizes])
            context.window_manager.clipboard = final
            self.report({'INFO'}, 'Paste the contents into a new line of the message board. Do not leave empty lines before or after other messages.')
            return {'FINISHED'}
        else:
            import json
            data = {
            ...
            self.report({'INFO'}, 'Replace the entirety of the update board with the contents of the clipboard. Do not leave a trace of old messages.')
            return {'FINISHED'}
...
class EMB_OT_rem_box(Operator):
    ...
    def invoke(self, context, event):
        return context.window_manager.invoke_confirm(self, event, title='Delete the text box?')
...
class EMB_OT_clear_gen(Operator):
    ...
    def invoke(self, context, event):
        return context.window_manager.invoke_confirm(self, event, title='Clear the message setup?')
...
class EMB_OT_adjust_preferences(Operator):
    ...
    def invoke(self, context, event):
        ...
        return context.window_manager.invoke_props_dialog(self, width=400, title='Properties')
...
class textbox_props(PropertyGroup):
    text: StringProperty(default='Sample Text', options={'TEXTEDIT_UPDATE'}, name='Text')
...
class emb_props(PropertyGroup):
    ...
    title: StringProperty(name='Title', description='The title for this latest update/message', default='Message Title', options={'TEXTEDIT_UPDATE'})
    message_type: EnumProperty(items=(
        ('MESSAGE', 'Is Message', 'Make the generation type for messages'),
        ('UPDATE', 'Is Update', 'Make the generation type for updates'),
    ), name='Message Type', default='MESSAGE')

    v_major: IntProperty(min=1)
